...
class HotDealClickAdapter(
...
) : HotDealClickPort, HotDealClickChecker, HotDealClickQueryPort {
    ...
    override fun findDealIdsByUserIdAndDealIds(userId: Long, dealIds: List<Long>): Set<Long> =
        hotDealClickRepository.findAllByUserIdAndDealIdIn(userId, dealIds)
            .map { it.dealId }
            .toSet()

    fun existsByDealIdAndUserId(dealId: Long, userId: Long): Boolean =
        hotDealClickRepository.existsByDealIdAndUserId(dealId, userId)
}
